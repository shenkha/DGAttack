...
def chatbot_gen(tokenizer, model, prompts: str, stop: Optional[list[str]] = None, system_prompts: str = None, device=None, config=None):
...
class Problem:
    ...
    def __init__(self,model, tokenizer, context, persona ,original_sentence, guided_sentence, device,max_len,task,acc_metric,bleu,rouge,meteor,ori_ref,model_name):
    ...
    def calculate_objectives(self, individual):
        if individual and individual.sentence and individual.guided_sentence:
            ...
            generated_responses = chatbot_gen(self.tokenizer, self.model, prompts, system_prompts=system_prompts,device=self.device, config=self.config)
            length = len(generated_responses.split())
...
class Evolution:
    ...
    def __init__(self, crossover_flag, file_path_gen, problem, num_of_generations=1000, num_of_individuals=100, num_of_tour_particips=2,
        ...
        self.model_name = problem.model_name
        self.model = problem.model
        self.tokenizer = problem.tokenizer
        self.config = problem.config
        self.persona = problem.persona
        self.original_context = problem.context
        self.device = problem.device
        self.response = problem.ori_ref
    ...
    def evolve(self):
        ...
        dialogue_his = self.persona + self.original_context
        ...
        #         ### Current Utterance:
        #         {self.problem.original_sentence}
        ...
        self.log_and_save_gen("G--{}".format(self.response))
